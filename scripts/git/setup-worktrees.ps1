...
# Part of Victorious MARKET Multi-AI Engineering Control System Specification v3 (§5.2, §12.1)
...
foreach ($r in $Roles) {
    ...
    try {
        git config user.name "$($r.Name)"
        git config user.email "$($r.Email)"
        git config vmarket.role "$($r.Id)"
...
APP_ENV=testing
APP_KEY=base64:TestKeyForMultiAiControlSystemIsolatedEnv32=
APP_DEBUG=true
...
Write-Host "`n[✓] All 8 AI worktrees successfully configured!" -ForegroundColor Green
