...
- **Status**: `FULFILLED`
- **Resolution**: Added `verification_code` and `pickup_verification_code` to `Order::$hidden`. Bare serialization (`$orders->toJson()`, `$detail->toArray()`) across all rider endpoints (`current-orders`, `all-orders`, `order-details`, `search`, `getOrderItem`) will strictly omit these OTP attributes.
- **Server Verification**:
  - `DeliveryManController@verify_order_delivery_otp` continues reading `verification_code` directly from the database attribute with constant-time equality check.
  - Customer order tracking explicitly unhides the code via `$order->makeVisible(...)` only when the caller is proven to be the verified customer owner.
- **Registry Entry**: Documented in `API_CONTRACT_REGISTRY.md` §5.
