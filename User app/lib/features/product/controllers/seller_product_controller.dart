...
class SellerProductController extends ChangeNotifier {
  ...
  Future<void> getSellerWiseBestSellingProductList(String slug, int offset) async {
    ApiResponseModel apiResponse = await sellerProductServiceInterface!.getSellerWiseBestSellingProductList(slug, offset.toString());
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      if(offset == 1){
        productModel = null;
  ...
  Future<void> getSellerWiseFeaturedProductList(String slug, int offset) async {
    ApiResponseModel apiResponse = await sellerProductServiceInterface!.getSellerWiseFeaturedProductList(slug, offset.toString());
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      if(offset == 1){
  ...
  Future<void> getSellerWiseRecommendedProductList(String slug, int offset) async {
    ApiResponseModel apiResponse = await sellerProductServiceInterface!.getSellerWiseRecomendedProductList(slug, offset.toString());
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      if(offset == 1){
  ...
  void setFilterApply(bool value, {bool isUpdate = true}) {
    _isFilterApply = value;
    if(isUpdate) {
      notifyListeners();
    }
  }
