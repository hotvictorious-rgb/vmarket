...
class AsterThemeHomeScreen extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload) async {
    ...
    if (authController.isLoggedIn()) {
      ...
      secondaryFutures
          .add(sellerProductController.getShopAgainFromRecentStore());
      if (orderController.orderModel == null ||
          (orderController.orderModel != null &&
              orderController.orderModel!.orders!.isEmpty) ||
          reload) {
        secondaryFutures
            .add(orderController.getOrderList(1, 'delivered', type: 'reorder'));
      }
    }

    if (reload) {
      await Future.wait(priorityFutures.map(
          (f) => f.catchError((e) => debugPrint('Priority load error: $e'))));
      await Future.wait(secondaryFutures.map(
          (f) => f.catchError((e) => debugPrint('Secondary load error: $e'))));
    } else {
      Future.wait(priorityFutures.map((f) =>
              f.catchError((e) => debugPrint('Priority load error: $e'))))
          .then((_) {
        Future.wait(secondaryFutures.map((f) =>
            f.catchError((e) => debugPrint('Secondary load error: $e'))));
      });
    }
  }
}
                        ...
                        children: [
                            ...
                            InkWell(
                              onTap: () async {
                                final Uri launchUri = Uri(
                                    scheme: 'tel',
                                    path: Provider.of<SplashController>(context,
                                            listen: false)
                                        .configModel!
                                        .companyPhone);
                          ...
                          Consumer<NotificationController>(
                            builder: (context, notificationProvider, _) {
                              final int unreadCount = notificationProvider
              ...
              child: Consumer<ProductController>(
                ...
                if (productController.findWhatYouNeedModel != null &&
                  ...
                  return Column(children: [
                    TitleRowWidget(
                        title: getTranslated('find_what_you_need', context)),
                    const SizedBox(height: Dimensions.paddingSizeSmall),
                    SizedBox(
                ...
                } else if (productController.isFindWhatYouNeedLoading &&
                    !productController.hasFindWhatYouNeedLoaded) {
