...
class ChatController extends ChangeNotifier {
  ...
  void pickMultipleMedia(bool isRemove,{int? index, bool openCamera = false}) async {
    ...
    if(isRemove) {
      ...
      final XFile? pickedImage = await ImageValidationHelper.validateAndPickImage(
          ...
          context: Get.context!,
          imageQuality: AppConstants.imageQuality, maxWidth: 400
      );
      ...
      for (var file in _pickedImageFiles) {
        if (
        [
          ...AppConstants.imageExtensions,
          ...AppConstants.videoExtensions,
      ...
      for (PlatformFile file in validatedFiles) {
        if (isVideoExtension(file.path ?? '')) {
          ...
          if (thumbnailPath != null && size <= (configModel?.systemGeneralFileUploadMaxSize ?? AppConstants.maxSizeOfASingleFile)) {
            ...
            showCustomSnackBarWidget('${getTranslated('maximum_file_size', Get.context!)} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB', Get.context!, snackBarType: SnackBarType.warning);
          }
        } else {
          ...
          if(value > (configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)) {
            showCustomSnackBarWidget('${getTranslated('maximum_image_size', Get.context!)} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB', Get.context!, snackBarType: SnackBarType.warning);
          } else {
  ...
  Future<bool> _isCrossMediaMaxLimit() async =>
      _pickedImageFiles.length == AppConstants.maxLimitOfTotalFileSent
          && await ImageValidationHelper.getMultipleImageSizeFromXFile(getXFileFromMediaFileModel(pickedMediaStored ?? []) ?? []) > AppConstants.maxLimitOfFileSentINConversation;
  ...
  SenderType getSenderType(Message? senderData) {
    if (senderData?.sentByCustomer ?? false) {
      return SenderType.customer;
    } else if (senderData?.sentBySeller ?? false) {
      return SenderType.seller;
    } else if (senderData?.sentByAdmin ?? false) {
      return SenderType.admin;
    } else if (senderData?.sentByDeliveryman ?? false) {
  ...
  Future<void> pickOtherFile(bool isRemove, {int? index}) async {
    ...
    if(!AppConstants.demo) {
      allowedFileExtensions.addAll(['rar', 'tar', 'targz', 'zip']);
    }
    ...
    if(isRemove){
      ...
      if(platformFile != null) {
        for (var file in platformFile) {
          ...
          if(!hasInvalidFile && fileSize > (Provider.of<SplashController>(Get.context!, listen: false).configModel?.systemGeneralFileUploadMaxSize ?? AppConstants.maxSizeOfASingleFile)) {
            showCustomSnackBarWidget('${getTranslated('maximum_file_size', Get.context!)} ${(Provider.of<SplashController>(Get.context!, listen: false).configModel?.systemGeneralFileUploadMaxSize ?? AppConstants.maxSizeOfASingleFile)}MB', Get.context!, snackBarType: SnackBarType.warning);
          } else if (
          allowedFileExtensions.contains(file.extension?.toLowerCase())) {
            validatedFiles.add(file);
          } else {
            hasInvalidFile = true;
          }
  ...
  bool _isCrossedMaxFileLimit(List<PlatformFile>? platformFile) =>
      pickedFiles?.length == AppConstants.maxLimitOfTotalFileSent
          && platformFile != null
          && ImageValidationHelper.getMultipleFileSizeFromPlatformFiles(platformFile) > AppConstants.maxLimitOfFileSentINConversation;
  ...
  String? getOnPressChatTime(Message currentConversation){
    if(currentConversation.id.toString() == _onMessageTimeShowID || currentConversation.id.toString() == _onImageOrFileTimeShowID){
      ...
      }else if(currentDate.weekday == todayConversationDateTime.weekday && DateConverter.countDays(todayConversationDateTime) <= 7){
  ...
  void resetIsSearchComplete({bool isUpdate = true}){
    _isSearchComplete = false;
