        ...
        <form action="{{route('admin.delivery-man.withdraw-update-status',[$details['id']])}}" method="POST" id="approval-note-form" data-message="{{ translate('want_to_approve_this_withdraw_request') . '?' }}" enctype="multipart/form-data">
            ...
            <div class="mt-5 d-none note-area" id="approval-note" >
                ...
                <div class="d-flex gap-2 pt-3 justify-content-center">
                    ...
                    <button type="button" class="btn btn-primary min-w-100px form-submit" data-form-id="approval-note-form" data-message="">{{translate('complete')}}</button>
                </div>
            </div>
        </form>
        <form action="{{route('admin.delivery-man.withdraw-update-status',[$details['id']])}}" method="POST" id="denial-note-form"  data-message="{{ translate('want_to_deny_this_withdraw_request') . '?' }}">
            @csrf
            <input name="approved" value="2" hidden="">
            <div class="mt-5 d-none note-area" id="denial-note">
                <h4 class="text-center mb-3">{{translate('denial_note')}}</h4>
                <textarea name="note" id="" class="form-control" rows="6" placeholder="{{translate('type_a_note_about_request_denial').'.'}}"></textarea>
                <div class="d-flex gap-2 pt-3 justify-content-center">
                    <button type="button" class="btn btn-secondary min-w-100px back-to-details mx-2"> {{translate('back')}}</button>
                    <button type="button" class="btn btn-primary min-w-100px form-submit" data-form-id="denial-note-form" data-message="">{{translate('complete')}}</button>
                </div>
            </div>
        </form>
    </div>
    <div class="mt-4 d-flex justify-content-center gap-3">
        <div class="withdraw-details">
            @if ($details['approved'] == 0)
                <button type="button" class="btn badge-danger text-bg-danger min-w-100px open-note mx-2" data-id="denial-note" data-message="{{translate('want_to_deny_this_withdraw_request').'?'}}">{{translate('deny')}}</button>
