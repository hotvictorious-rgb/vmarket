                    ...
                    <div class="row g-4">
                        ...
                        @if ($seller['status']!="pending")
                            ...
                            @php
                                ...
                                $kycSummary = $kycService->getKycSummary($seller);
                            @endphp
                            <div class="col-12 mt-3">
                                <div class="card border border-primary-light">
                                    <div class="card-header bg-soft-primary d-flex justify-content-between align-items-center flex-wrap gap-2">
                                        <div class="d-flex align-items-center gap-2">
                                            <i class="fi fi-sr-shield-check fs-18 text-primary"></i>
                                            <h4 class="mb-0 text-capitalize">{{ translate('Nigerian_KYC_&_Identity_Verification') }}</h4>
                                        </div>
                                        <div>
                                            @if(($seller['kyc_status'] ?? 'pending') == 'verified')
                                                <span class="badge badge-success px-3 py-2 fs-13"><i class="fi fi-sr-badge-check"></i> {{ translate('Verified_Merchant') }} 🛡️</span>
                                            @elseif(($seller['kyc_status'] ?? 'pending') == 'submitted')
                                                <span class="badge badge-warning px-3 py-2 fs-13"><i class="fi fi-sr-hourglass-end"></i> {{ translate('Submitted_For_Review') }} ⏳</span>
                                            @elseif(($seller['kyc_status'] ?? 'pending') == 'rejected')
                                                <span class="badge badge-danger px-3 py-2 fs-13">{{ translate('Rejected') }}</span>
                                            @else
                                                <span class="badge badge-secondary px-3 py-2 fs-13">{{ translate('Unverified') }} ⚠️</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <div class="row g-3">
                                            <div class="col-md-4">
                                                <div class="p-3 bg-light rounded">
                                                    <h5 class="fs-14 fw-bold mb-2">{{ translate('National_Identity_Number_(NIN)') }}</h5>
                                                    <p class="mb-1 text-muted">{{ translate('NIN') }}: <strong class="text-dark">{{ $seller['nin'] ?? translate('Not_Provided') }}</strong></p>
                                                    @if(!empty($seller['nin_document']))
                                                        <a href="{{ asset('storage/app/public/seller/kyc/' . $seller['nin_document']) }}" target="_blank" class="btn btn-xs btn-outline-primary mt-2">
                                                            <i class="fi fi-rr-eye"></i> {{ translate('View_NIN_Document') }}
                                                        </a>
                                                    @else
                                                        <span class="text-muted fs-12">{{ translate('No_document_attached') }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="p-3 bg-light rounded">
                                                    <h5 class="fs-14 fw-bold mb-2">{{ translate('CAC_Registration_(Optional)') }}</h5>
                                                    <p class="mb-1 text-muted">{{ translate('CAC_Number') }}: <strong class="text-dark">{{ $seller['cac_number'] ?? translate('Not_Provided') }}</strong></p>
                                                    @if(!empty($seller['cac_document']))
                                                        <a href="{{ asset('storage/app/public/seller/kyc/' . $seller['cac_document']) }}" target="_blank" class="btn btn-xs btn-outline-primary mt-2">
                                                            <i class="fi fi-rr-eye"></i> {{ translate('View_CAC_Certificate') }}
                                                        </a>
                                                    @else
                                                        <span class="text-muted fs-12">{{ translate('No_CAC_attached') }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="p-3 bg-light rounded">
                                                    <h5 class="fs-14 fw-bold mb-2">{{ translate('Bank_Name_Cross_Match') }}</h5>
                                                    <div class="d-flex align-items-center gap-2 mb-1">
                                                        <span class="badge {{ $kycSummary['name_match_score'] >= 80 ? 'badge-soft-success text-success' : ($kycSummary['name_match_score'] >= 50 ? 'badge-soft-warning text-warning' : 'badge-soft-danger text-danger') }} fs-14 fw-bold">
                                                            {{ $kycSummary['name_match_score'] }}% Match
                                                        </span>
                                                        <span class="fs-12 text-muted">({{ ucfirst($kycSummary['match_type']) }})</span>
                                                    </div>
                                                    <p class="mb-0 fs-12 text-muted">{{ $kycSummary['match_details'] }}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                                            @if(($seller['kyc_status'] ?? 'pending') != 'verified')
                                                <form action="{{ route('admin.vendors.kyc-status', ['id' => $seller['id']]) }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="status" value="verified">
                                                    <button type="submit" class="btn btn-success">
                                                        <i class="fi fi-sr-check"></i> {{ translate('Approve_KYC_&_Grant_Verified_Badge') }} 🛡️
                                                    </button>
                                                </form>
                                            @endif

                                            @if(($seller['kyc_status'] ?? 'pending') != 'rejected')
                                                <form action="{{ route('admin.vendors.kyc-status', ['id' => $seller['id']]) }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="status" value="rejected">
                                                    <button type="submit" class="btn btn-outline-danger">
                                                        <i class="fi fi-sr-cross"></i> {{ translate('Reject_KYC') }}
                                                    </button>
                                                </form>
                                            @endif
                                        </div>
