...
@section('content')
    <div class="content container-fluid">
        <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
            ...
            <a href="{{ route('frontend.blog.details', ['slug' => $blog['slug'], 'source' => 'draft']) }}"
               target="_blank" class="btn btn-outline-primary"  data-bs-toggle="tooltip" data-placement="bottom"
               title=""
            >
        ...
        <form action="{{ route('admin.blog.update', [$blog['id']]) }}" method="POST" enctype="multipart/form-data" id="blog-ajax-form">
            ...
            <div class="card card-body mb-3">
                <div class="row gy-4 service-description-wrapper">
                    <div class="col-lg-6 d-flex">
                        <div class="bg-section rounded p-12 p-sm-20 flex-fill">
                            <div class="form-group">
                                <div class="d-flex justify-content-between align-items-center gap-3 mb-2">
                                    <label for="name" class="form-label mb-0">
                                        {{ translate('Category') }}
                                        <span class="tooltip-icon" data-bs-toggle="tooltip" data-placement="right" title=""
                                              data-bs-title="{{ translate('select_a_category_from_the_dropdown_menu_to_assign_this_blog.') }} {{ translate('if_no_categories_are_available_or_want_to_add_a_new_category_please_add_it_from_the_manage_category_section') }}">
                                              <i class="fi fi-sr-info"></i>
                                        </span>
                                    </label>
                                    <a data-bs-toggle="offcanvas" href="#offcanvasCategory" class="fw-medium user-select-none category-sidebar-toggle">
                            ...
                            <div class="form-group mb-0">
                                <label for="name" class="form-label">
                                    ...
                                    <span class="tooltip-icon" data-bs-toggle="tooltip" data-placement="top" title=""
                                          data-bs-title="{{ translate('pick_the_date_that_you_want_to_show_for_customers_as_blog_publishing_date') }}">
                                        <i class="fi fi-sr-info"></i>
                                    </span>
                                </label>
                                <div class="position-relative">
                                    <input type="date" name="publish_date" class="form-control cursor-pointer" value="{{ $draftData['publish_date'] ?? $blog->publish_date->format('Y-m-d') }}" autocomplete="off">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 d-flex">
                        <div class="bg-section rounded p-12 p-sm-20 flex-fill">
                            <div class="d-flex flex-column gap-20 justify-content-between h-100">
                                ...
                                <div class="upload-file">
                                    ...
                                    <label class="upload-file__wrapper w-325">
                                        ...
                                        <img class="upload-file-img" loading="lazy"
                                             src="{{ $blog->is_published ? getStorageImages(path: $blog?->thumbnail_full_url, type:'backend-product') : getStorageImages(path: $blog?->draft_thumbnail_full_url, type:'backend-product') }}"
                                             data-default-src="{{ $blog->is_published ? getStorageImages(path: $blog?->thumbnail_full_url, type:'backend-product') : getStorageImages(path: $blog?->draft_thumbnail_full_url, type:'backend-product') }}"
                                             alt="">
                                    </label>
                                    <div class="overlay">
                                ...
                                <p class="fs-10 mb-0 text-center">{{getFileUploadFormats(skip: '.svg')}} {{ translate('Image_size_:_Max_'). getFileUploadMaxSize()."MB" }} <span class="fw-medium">(325 x 100 px)</span></p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        @foreach($languages as $lang)
                            <div class="{{$lang != $defaultLanguage ? 'd-none':''}} form-system-language-form" id="{{$lang}}-form">
                                <div class="form-group mb-4">
                                    <div class="d-flex justify-content-between align-items-center gap-2 mb-2">
                                        <label for="name" class="mb-2 fw-medium text-capitalize">{{ translate('title')}}
                                            ({{strtoupper($lang)}})
                            ...
                            <div class="form-group mb-0 {{$lang != $defaultLanguage ? 'd-none':''}} form-system-description-language-form" id="{{ $lang}}-description-form">
                                ...
                                <div class="outline-wrapper" id="editor-container-{{ getLanguageCode($lang)}}">
                                    ...
                                    <textarea name="description[{{$lang}}]" id="description-{{getLanguageCode($lang)}}" style="display:none;" >{!! $defaultLanguage == $lang ? ($draftData['description'] ?? $blog->title):  $translate[$lang]['description'] ?? '' !!}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="d-flex flex-wrap gap-2 gap-sm-3 justify-content-end mt-4">
                <button type="button" id="reset"
                        class="btn btn-secondary min-w-120 reset-form clear-draft">{{ translate('clear_draft') }}</button>
                <button type="button" class="btn btn-outline-primary min-w-120 save-draft">{{ translate('Save_to_Draft') }}</button>
                <button type="button" class="btn btn-primary min-w-120" data-bs-toggle="modal" data-bs-target="#toggle-status-publish-modal">{{ translate('Update_Publish') }}</button>
            </div>
        </form>
    </div>
