...
class VendorController extends BaseController
{
    ...
    public function withdrawStatus(Request $request, $id): RedirectResponse
    {
        ...
        if ($request['approved'] == 1) {
            if (!$request->hasFile('proof_of_payment')) {
                ToastMagic::error(translate('Proof_of_payment_screenshot_is_required_when_approving_vendor_payout'));
                return back();
            }
    ...
    public function updateKycStatus(Request $request, $id): RedirectResponse|JsonResponse
    {
        $status = $request->input('status', 'verified');
        $this->vendorRepo->update(id: $id, data: ['kyc_status' => $status]);

        if ($request->ajax()) {
            return response()->json(['status' => true, 'message' => translate('KYC status updated successfully.')]);
        }

        ToastMagic::success(translate('KYC_status_updated_successfully'));
        return back();
    }
