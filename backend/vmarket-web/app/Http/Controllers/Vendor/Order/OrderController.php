...
class OrderController extends BaseController
{
    ...
    ): JsonResponse
    {
        ...
        // Generic vendor status endpoint does not have delivery completion authority for marketplace orders.
        // Delivery orders require verified doorstep customer OTP, and pickup orders require verified in-shop handover OTP via InShopHandoverController.
        if (\App\Utils\OrderManager::isVictoriousMarketplaceOrder($order) && $request['order_status'] === 'delivered') {
            return response()->json([
                'status' => 0,
                'message' => translate('Generic vendor status endpoint cannot mark marketplace orders delivered. Delivery orders require doorstep customer OTP verification, and pickup orders require in-shop handover OTP verification.'),
            ], 403);
        }
