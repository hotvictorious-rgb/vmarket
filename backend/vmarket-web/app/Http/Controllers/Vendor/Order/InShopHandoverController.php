...
class InShopHandoverController extends Controller
{
    ...
    public function verifyPickupOtp(Request $request): JsonResponse|RedirectResponse
    {
        ...
        // [AI] 100% Motorized Rider Handover Enforcement:
        // Customer self-pickup is strictly disabled in Victorious MARKET.
        // Handover in-shop is restricted exclusively to authorized delivery riders with an assigned rider ID.
        if ($order->order_type === 'pickup' || $order->delivery_type === 'self_pickup' || empty($order->delivery_man_id)) {
            $message = translate('Customer self-pickup is disabled. Handover is restricted exclusively to authorized delivery riders with an assigned rider ID.');
        ...
        $actionLabel = translate('Custody_transferred_to_rider_successfully!');
