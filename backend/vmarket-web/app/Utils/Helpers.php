    ...
    public static function createDefaultShop()
    {
        ...
        if (!$shop) {
            ...
            $data = [
                'seller_id' => 0,
                'author_type' => 'admin',
                'name' => getWebConfig('company_name') ?? 'Victorious MARKET',
                'slug' => Str::slug(getWebConfig('company_name') ?? 'victorious-market') . '-' . rand(1000, 9999),
                'contact' => getWebConfig('company_phone') ?? '',
                'address' => getWebConfig('shop_address') ?? '',
                'image' => (is_array($companyFavIcon) ? ($companyFavIcon['key'] ?? '') : ''),
                'image_storage_type' => (is_array($companyFavIcon) ? ($companyFavIcon['storage'] ?? 'public') : 'public'),
                'banner' => (is_array($shopBanner) ? ($shopBanner['key'] ?? '') : ''),
                'banner_storage_type' => (is_array($shopBanner) ? ($shopBanner['storage'] ?? 'public') : 'public'),
                'offer_banner' => (is_array($offerBanner) ? ($offerBanner['key'] ?? '') : ''),
                'offer_banner_storage_type' => (is_array($offerBanner) ? ($offerBanner['storage'] ?? 'public') : 'public'),
                'bottom_banner' => (is_array($bottomBanner) ? ($bottomBanner['key'] ?? '') : ''),
                'bottom_banner_storage_type' => (is_array($bottomBanner) ? ($bottomBanner['storage'] ?? 'public') : 'public'),
                'vacation_duration_type' => (is_array($vacationConfig) ? ($vacationConfig['vacation_duration_type'] ?? 'until_change') : 'until_change'),
                'vacation_start_date' => (is_array($vacationConfig) ? ($vacationConfig['vacation_start_date'] ?? null) : null),
                'vacation_end_date' => (is_array($vacationConfig) ? ($vacationConfig['vacation_end_date'] ?? null) : null),
                'vacation_note' => (is_array($vacationConfig) ? ($vacationConfig['vacation_note'] ?? '') : ''),
                'vacation_status' => (is_array($vacationConfig) ? ($vacationConfig['status'] ?? 0) : 0),
                'temporary_close' => (is_array($temporaryClose) ? ($temporaryClose['status'] ?? 0) : 0),
            ];
